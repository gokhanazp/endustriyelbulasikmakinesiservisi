...
// =============================================================
//  ÖNCELİKLİ MARKALAR ve MARKA × İLÇE MATRİSİ
// =============================================================
// SEO raporu: rekabetin en yoğun olduğu 4 marka öne çıkarılır ve bu
// markalar için ilçe kırılımlı sayfalar üretilir.
...
// Endüstriyel mutfak yoğunluğuna göre öncelikli 12 ilçe
export const matrixDistrictSlugs = [
  "sisli", "beyoglu", "fatih", "besiktas", "bakirkoy", "bahcelievler",
  "kucukcekmece", "esenyurt", "basaksehir", "zeytinburnu", "bagcilar", "beylikduzu",
];

export const matrixDistricts = () =>
  matrixDistrictSlugs.map((s) => districts.find((d) => d.slug === s)).filter(Boolean);

export const matrixPath = (brand, district) =>
  `/${district.slug}-${brand.slug}-bulasik-makinesi-servisi`;

// Bir markanın ilçe sayfaları / bir ilçenin marka sayfaları
export const matrixForBrand = (brandSlug) =>
  priorityBrandSlugs.includes(brandSlug) ? matrixDistricts() : [];
export const matrixForDistrict = (districtSlug) =>
  matrixDistrictSlugs.includes(districtSlug) ? priorityBrands() : [];

export function allMatrixPairs() {
  const out = [];
  for (const b of priorityBrands()) {
    for (const d of matrixDistricts()) out.push({ brand: b, district: d });
  }
  return out;
}
...
export function resolveSlug(slug) {
  ...
  if (slug.endsWith(brandSuffix)) {
    ...
    // 1) Marka × ilçe matris sayfası: /{ilce}-{marka}-bulasik-makinesi-servisi
    for (const d of matrixDistricts()) {
      if (part.startsWith(`${d.slug}-`)) {
        const brandPart = part.slice(d.slug.length + 1);
        const brand = brands.find(
          (b) => b.slug === brandPart && priorityBrandSlugs.includes(b.slug)
        );
        if (brand) return { type: "matrix", brand, district: d };
      }
    }
...
export function allDynamicSlugs() {
  return [
    ...
    ...allMatrixPairs().map(({ brand, district }) => `${district.slug}-${brand.slug}-bulasik-makinesi-servisi`),
  ];
}
